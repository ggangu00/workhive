   ...
   <div class="w-auto h-auto collapse navbar-collapse max-height-vh-100 h-100" id="sidenav-collapse-main">
      <ul class="navbar-nav">
         ...
         <li class="nav-item" v-for="(item, i) in menus" :key="i">
            <sidenav-collapse :aria-controls="''" :collapse="false" :collapseRef="item.url" :navText="item.menuNm">
               <template v-slot:icon>
      ...
      :aria-controls="''"
       ...
       >
         ...
         <template v-slot:list>
            <div class="sub-item">
               <li class="sub-item" @click="movePage('bulletinList')">공지사항</li>
               <li class="sub-item" @click="movePage('bulletinList')">사내게시판</li>                        
            </div>
         </template>
      </sidenav-collapse>
   </li>
