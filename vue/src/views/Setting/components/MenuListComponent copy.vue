<template>
   <div :class="menu.parentMenuCd == null ? 'bottom-line' : ''">
      <div class="menu-title" @click.self="toggleMenu">
         <div>
            <div class="form-check form-check-inline">
               ...
               <span class="ms-2 font-14" v-if="menu.subMenus.length > 0">
                  ({{ subMenusSelected(menu) }}/{{ menu.subMenus.length }})
               </span>
            </div>
         </div>
      ...
      <div v-if="menu.open && menu.subMenus.length > 0" class="submenu px-4" >
         <MenuListComponent v-for="(subItem, idx) in menu.subMenus" :key="idx" :item="subItem" :isMenuEditing="isMenuEditing"/>
...
<script setup>
   ...
   const props = defineProps({
      ...
      isMenuEditing: { type: Boolean, required: true },
   });

   const menu = props.item;
   // 필요하다면 computed로 반응형 보장
